...
def clacification(image, mask, edge):
    type = 0
    mask = mask * 255
    ...
    for calcification_contour in calcification_contours:
        ...
        if type == 1:
            ...
            # print(shape)
                ...
                cv.drawContours(result, [calcification_contour], -1, (0, 128, 255), 2)
                shape = "内部钙化（微钙化）"
            add_shape(shape)
    ...
    colors = [(0, 0, 128), (128, 128, 255), (0, 0, 255), (0, 128, 255), (0, 255, 0), (0, 128, 0), (50, 205, 50), (128, 0, 0),
              (64, 0, 0), (255, 0, 0)]  # 标签的颜色列表
    labels = ['内部钙化（斑点钙化）', '内部钙化（明亮钙化）', '内部钙化（弧形钙化）', '内部钙化（微钙化）', '边缘钙化（环状）', '边缘钙化（弧形）',
    ...
    for i in range(len(colors)):
        ...
        font = ImageFont.truetype("./calcification/SimSun.ttf", 20, encoding="utf-8")  # 参数1：字体文件路径，参数2：字体大小
